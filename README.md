...
Upon configuring the authentication to the root account of the organization, it is necessary to install the SDK dependencies:

```shell
pip install huaweicloudsdkall 
```

Having installed the SDK dependencies, it is also necessary to set the variable <code>bucket_name</code> on the <code>./tf_child/variables.tf</code> file in order to set-up the Config Resource Recorder module of the Terraform file. 
